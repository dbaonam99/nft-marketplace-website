              ...
              <div className="donnot-miss-widget">
                          ...
                          <NavLink to="/discover" className={isLightMode ? "filter-item text-dark" : "filter-item"}>
                              {isLightMode ?
                                <div className="d-flex align-items-center">
                                    <i className="fa fa-list mr-2" aria-hidden="true"></i>{t("itemDetails.cryptoArt")}
                                </div> :
                                <><img src={iconsf1} alt="" />{t("itemDetails.cryptoArt")}</>
                              }
                          </NavLink>
                      </div>
                      <h4 className={isLightMode ? "text-dark" : ""}>Floyd Mayweather Jr.</h4>
                  </div>
                  <div className={isLightMode ? "mb-15 text-muted" : "mb-15 gray-text"}><span className={isLightMode ? "text-dark mr-15" : "w-text mr-15"}>{t("common.currentPrice")} 0.34 ETH </span><span className={isLightMode ? "mb-15 text-muted" : "mb-15 gray-text"}>$534.22</span> 1/10</div>
                  <div className="details-list">
                      {details && details.map((item , i) => (
                        <p className={isLightMode ? "text-muted" : ""} key={i}>{t(item.text1)}: <span className={isLightMode ? "text-dark" : ""}>{item.text2}</span></p>  
                      ))}
                  </div>
                  <div className="author-item mb-30">
                      ...
                      <div className="author-info">
                          ...
                          <p className={isLightMode ? "author-earn mb-0 text-muted" : "author-earn mb-0"}>{t("common.itemOwner")}</p>
                      </div>
                  </div>
                  <div className={isLightMode ? "highest-bid bt-bg-light" : "highest-bid"}>
                      <h5 className={isLightMode ? "text-dark mb-15" : "w-text mb-15"}>{t("common.highestBid")}</h5>
                  ...
                  <NavLink to="#test-popup" className="open-popup-link more-btn width-100 mt-30">{t("common.purchaseNow")}</NavLink>
              </div>
          </div>
      </div>
    </>

...
function ItemTimeline({ date, time, actionType, tokenId }) {
    ...
    const { data: nftDetail, isLoading: nftDetailLoading } =
        useGetNFTDetailQuery(
            actionType === "createToken"
                ? ""
                : (
                    !isAuction(actionType) ? tokenId : ""
                )
        );
    ...
    const description = useMemo(() => {
        switch (actionType) {
            ...
            case "createMarket":
                return (
                    <>
                        {t("activity.saledToken")} <NavLink to={`/item-details/${tokenId}`}>{data?.name}</NavLink> {<>{t("common.for")} <span className={isLightMode ? "text-dark" : "w-text"}>{data?.price} UIT</span></>}
                    </>
                )
            case "buyToken":
                return (
                    <>
                        {t("activity.buyToken")} <NavLink to={`/item-details/${tokenId}`}>{data?.name}</NavLink> {<>{t("common.for")} <span className={isLightMode ? "text-dark" : "w-text"}>{data?.price} UIT</span></>}
                    </>
                )
            case "sellToken":
                return (
                    <>
                        {t("activity.sellToken")} <NavLink to={`/item-details/${tokenId}`}>{data?.name}</NavLink> {<>{t("common.for")} <span className={isLightMode ? "text-dark" : "w-text"}>{data?.price} UIT</span></>}
                    </>
                )
            case "startAuction":
                return (
                    <>
                        {t("activity.startAuction")} <NavLink to={`/item-details/${tokenId}?auction=true`}>{data?.name}</NavLink> {<>{t("common.withStartPrice")} <span className={isLightMode ? "text-dark" : "w-text"}>{data?.startingPrice} UIT</span></>}, {<>{t("common.biddingStep")} <span className={isLightMode ? "text-dark" : "w-text"}>{data?.biddingStep} UIT</span></>}
                    </>
                )
            case "bid":
                return (
                    <>
                        {t("activity.bid")} <NavLink to={`/item-details/${tokenId}?auction=true`}>{data?.name}</NavLink>
                    </>
                )
            default:
                break;
            ...
            {isLoading ?
                ...
                <li>
                    ...
                    <div className="timelineDate">
                        <p><i className="fa fa-calendar mr-5p"></i>{date}</p>
                        <p><i className="fa fa-clock-o mr-5p"></i>{time}</p>
                    </div>
                    <div className="timelineWork">
                        <h6 className={isLightMode ? "text-dark" : ""}>{title}</h6>
                        <span className={isLightMode ? "text-muted" : ""}>{description}</span>
                    </div>
                </li>
